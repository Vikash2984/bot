...
const handleResize = () => {
    ...
    const heightDifference = STATE.originalWrapperHeight - newHeight;
    const isKeyboardVisible = heightDifference > 150; // Arbitrary threshold

    if (isKeyboardVisible !== STATE.isKeyboardVisible) {
        STATE.isKeyboardVisible = isKeyboardVisible;
        if (isKeyboardVisible) {
            CHAT_WRAPPER.style.height = `${newHeight}px`;
            CHAT_MESSAGE_COLUMN_WRAPPER.style.paddingBottom = '120px';
            scrollToBottomOfMessages();
        } else {
            CHAT_WRAPPER.style.height = '100%';
            CHAT_MESSAGE_COLUMN_WRAPPER.style.paddingBottom = '70px';
        }
    }
...
const init = () => {
    STATE.originalWrapperHeight = window.innerHeight;
    ...
    // Prevent scrolling when focusing on input field
